...
type QuotaService interface {
	// CheckQuota 检查配额是否足够
	CheckQuota(ctx context.Context, userID int64, quotaType entities.QuotaType, value float64) (*QuotaCheckResult, error)
	
	// ConsumeQuota 消费配额
	ConsumeQuota(ctx context.Context, userID int64, quotaType entities.QuotaType, value float64) error
	
	// GetUserQuotas 获取用户配额列表
	GetUserQuotas(ctx context.Context, userID int64) ([]*entities.Quota, error)
	
	// GetQuotaUsage 获取配额使用情况
	GetQuotaUsage(ctx context.Context, userID int64, quotaType entities.QuotaType, period entities.QuotaPeriod) (*QuotaUsageInfo, error)
	
	// CreateQuota 创建配额
	CreateQuota(ctx context.Context, userID int64, quotaType entities.QuotaType, period entities.QuotaPeriod, limit float64) (*entities.Quota, error)
	...
	// ResetQuota 重置配额使用情况
	ResetQuota(ctx context.Context, userID int64, quotaType entities.QuotaType, period entities.QuotaPeriod) error
	
	// GetQuotaStatus 获取配额状态
	GetQuotaStatus(ctx context.Context, userID int64) (*QuotaStatus, error)
...
type QuotaUsageInfo struct {
	...
	Period      entities.QuotaPeriod `json:"period"`
	Limit       float64   `json:"limit"`
	Used        float64   `json:"used"`
	Remaining   float64   `json:"remaining"`
